...
app.delete('/countries/:id', admin, function (req, res) {
  log('Request DEL /countries/%s %j', req.params.id, req.body);

  api.country.remove(req.params.id, function (err, id) {
    if (err) return handleError(err, req, res);

    log('Deleted country %s', id.id);
