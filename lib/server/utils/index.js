...
exports.handleError = function handleError(err, req, res) {
  log("Error found: %j", err);
  res.json({ error: err });
}

/**
 * Converts a normal YouTube URL in one that can be used when embedding a video in an iframe
 *
 * @param {String} u A YouTube URL
 * @return {String} An embeddable YouTube URL
 * @api public
 */

exports.getEmbeddableYouTubeURL = function getEmbeddableYoutubeURL(u) {
  var uri = url.parse(u);
  var params = qs.parse(uri.query);
  return params.v ? uri.protocol + '//www.youtube.com/embed/' + params.v : u;
}

exports.OpenGraph = require('./open-graph')
exports.accepts = require('./accepts')
