...
exports.filter = function filter(options, fn) {
  ...
  if (options.technologies) {
    query.technology = { $in: options.technologies }
  }
  if (options.countries) {
    query.country = { $in: options.countries }
  }
  ...
  if (options.sortBy) {
    var order = options.sortOrder === 'asc' ? '' : '-';
    var sort = order + options.sortBy;
    job.sort(sort);
  }

  log('Looking for all applications with filters: %j, sort: %s', query, order + options.sortBy)

  job.exec(function(err, apps) {
    if (err) {
      log('Found error: %j', err);
      return fn(err);
    }
