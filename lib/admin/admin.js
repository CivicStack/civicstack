...
function valid(ctx, next) {
  ...
  if (/apps|tags|categories|countries/.test(section)) return next();
  if (/(apps|tags|categories|countries)\/create/.test(section)) return next();
  if (/(apps|tags|categories|countries)\/[a-z0-9]{24}\/?$/.test(section)) return next();
  page.redirect('/');
}
