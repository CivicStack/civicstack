...
function valid(ctx, next) {
  ...
  if (/apps|tags|countries|technologies|licenses/.test(section)) return next();
  if (/(apps|tags|countries|technologies|licenses)\/create/.test(section)) return next();
  if (/(apps|tags|countries|technologies|licenses)\/[a-z0-9]{24}\/?$/.test(section)) return next();
  page.redirect('/');
}
