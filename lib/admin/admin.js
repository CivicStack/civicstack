...
function valid(ctx, next) {
  ...
  if (/apps|tags|categories/.test(section)) return next();
  if (/(apps|tags|categories)\/create/.test(section)) return next();
  if (/(apps|tags|categories)\/[a-z0-9]{24}\/?$/.test(section)) return next();
  page.redirect('/');
}
