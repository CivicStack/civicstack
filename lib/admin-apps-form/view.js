...
var AdminAppForm = Ractive.extend({
  ...
  onrender: function () {
    ...
    this.set('links', app.links.map(function (link, i) {
      return { value: link, index: i, editing: false };
    }));
    ...
    this.on('removeLink', function (ev) {
      ...
      var currentValue = ev.context.index;
      var idx = indexOf(this.get('links'), function (v) {
        return v.index === currentValue;
      });
    ...
    function nextIndex () {
      ...
      console.dir(self.get('links'));

      return max(self.get('links')) + 1;
    }

    this.on('addLink', function (ev) {
      this.push('links', { value: 'http://', editing: true, index: nextIndex() });
      this.set('adding', true);
    });
